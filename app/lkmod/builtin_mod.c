...
// Symbols produced by objcopy -I binary on mods/hello/hello.so
extern const unsigned char _binary_mods_hello_hello_so_start[];
extern const unsigned char _binary_mods_hello_hello_so_end[];

static lkmod_module_t *g_builtin_mod;

static void lkmod_builtin_init(uint level) {
    (void)level;
    const void *blob = (const void *)_binary_mods_hello_hello_so_start;
    size_t len = (size_t)(_binary_mods_hello_hello_so_end - _binary_mods_hello_hello_so_start);
