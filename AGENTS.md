...
- Build C module: `make -C mods/hello CC=aarch64-elf-gcc`
- Build C++ module: `make -C mods/hello_cpp CXX=aarch64-elf-g++`
- Inject blob into RAM at a known address (when QEMU started with `--gdb`):
  - `scripts/run-lk-aarch64.sh --gdb` then in another shell: `scripts/lkmod-inject.py --file mods/hello/hello.so --addr 0x44000000`
- In the LK console:
...
- The build embeds `mods/hello/hello.so` into the kernel image and auto-loads it at boot (LK init hook at `APPS`).
- No extra steps if you have the cross toolchain; the build will `make -C mods/hello` and objcopy the blob.
